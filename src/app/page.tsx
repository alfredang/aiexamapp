        ...
        <div className="container-app relative grid gap-10 py-16 md:grid-cols-[1.2fr_1fr] md:py-24">
          <div>
            ...
            <form action="/practice-exams" className="mt-8 flex max-w-lg items-center gap-2 rounded-full border border-slate-200 bg-white p-1.5 shadow-card">
              ...
              <input name="q" placeholder="Search exams (e.g. SAA-C03, AZ-900)" className="flex-1 bg-transparent px-2 py-2 text-sm outline-none" />
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-6">
          {vendors.map(v => (
            <Link key={v.id} href={`/practice-exams/${v.slug}`} className="card-hover flex flex-col items-center justify-center px-4 py-6 text-center">
              <div className="mb-2 flex h-12 w-12 items-center justify-center rounded-md bg-gradient-to-br from-blue-50 to-purple-50 text-blue-700 font-semibold">
                {v.name.slice(0, 2).toUpperCase()}
              </div>
              <div className="text-sm font-medium">{v.name}</div>
              <div className="text-xs text-slate-500">{v._count.exams} exam{v._count.exams === 1 ? '' : 's'}</div>
            </Link>
          ))}
        </div>
      </section>
