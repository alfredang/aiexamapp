...
import { seedVault } from '@/lib/seed/vault-questions';

export const runtime = 'nodejs';

/**
 * One-shot admin endpoint to seed the HashiCorp Vault Associate (002)
 * bundle (vendor + 3 practice exams + 195 questions + bundle) into the
 * current database. Idempotent — safe to call repeatedly; rewrites
 * questions tagged `generatedBy: 'manual:vault-seed'`.
 *
...
export async function POST() {
  ...
  await db.adminLog.create({
    data: {
      ...
      action: 'seed.vault',
      targetType: 'Bundle',
      targetId: 'hashicorp-vault-associate',
