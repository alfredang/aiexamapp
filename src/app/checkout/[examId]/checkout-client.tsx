...
export function CheckoutClient({ examId, tier }: { examId: string; tier: Tier }) {
  ...
  // When the available methods load, default to the first enabled one.
  useEffect(() => {
    if (!methods) return;
    ...
    <div className="space-y-4">
      ...
      <div className={addressId ? '' : 'pointer-events-none opacity-50'}>
          ...
          <PayPalScriptProvider options={{ clientId, currency: 'USD', intent: 'capture' }}>
            <PayPalButtons
              ...
              onApprove={async (data) => {
                const r = await fetch('/api/paypal/capture', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ paypalOrderId: data.orderID }) });
        ...
        {method === 'PAYNOW' && (
          <button
            type="button"
            disabled={!addressId || paynowBusy}
            onClick={payWithPaynow}
            className="btn-primary-grad w-full"
          >
