    ...
    <div>
      ...
      <div className="mt-2 flex gap-2 text-sm">
        {(['DRAFT', 'PUBLISHED', 'ARCHIVED'] as const).map(s => (
          <a key={s} href={`?status=${s}`} className={`badge ${status === s ? 'bg-blue-50 text-blue-700' : ''}`}>{s}</a>
        ))}
      </div>
          ...
          return (
            <div key={q.id} className="card p-4">
              ...
              <ul className="mt-2 space-y-1 text-sm">
                {opts.map(o => <li key={o.id} className={correctIds.includes(o.id) ? 'text-green-700' : 'text-slate-700'}>{correctIds.includes(o.id) ? '✓ ' : '○ '}{o.text}</li>)}
              </ul>
