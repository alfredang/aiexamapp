/* Refill comptia-security-plus-p1 ONLY (admin emptied it on prod).
 * Slug-keyed, idempotent, question-only — same proven pattern.
 *
 * SAFE: INSERT INTO "Question" only; NOT EXISTS(examId,stem); no
 * Exam/Bundle/Vendor writes; never touches published/deletedAt; no
 * deletes. status = PUBLISHED. Scoped to comptia-security-plus-p1.
 */
...
const T = '$sp1$';
const SLUG = 'comptia-security-plus-p1';
...
async function main() {
  ...
  const ts = '20260519230000';
  const dir = path.join('prisma', 'migrations', `${ts}_securityplus_p1_refill`);
