/**
 * One-shot seed: Anthropic Claude Certified Architect — Foundations (CCA-F)
 *
 *   npx tsx scripts/seed-cca-foundations.ts
 *
 * Idempotent on the Exam (upsert by slug) and on Questions (skips if any
 * question already exists for the exam — re-run after deleting questions).
 *
 * Source: 12 sample questions from the official CCA-F exam guide PDF
 * (Anthropic, Feb 2025, v0.1). Domain weights match the PDF blueprint.
 */
import { PrismaClient, QStatus, QType } from '@prisma/client';

const db = new PrismaClient();

const VENDOR_SLUG = 'anthropic';
...
const QUESTIONS: Q[] = [
  {
    ...
    stem:
      'You are building a customer support resolution agent using the Claude Agent SDK. The agent has access to MCP tools (get_customer, lookup_order, process_refund, escalate_to_human). Production data shows that in 12% of cases, your agent skips get_customer entirely and calls lookup_order using only the customer\'s stated name, occasionally leading to misidentified accounts and incorrect refunds. What change would most effectively address this reliability issue?',
    options: [
  ...
  },
  {
    ...
    domain: 'Agentic Architecture & Orchestration',
    stem:
      'You are running a multi-agent research system on the topic "impact of AI on creative industries." Each subagent completes successfully: the web search agent finds relevant articles, the document analysis agent summarizes papers correctly, and the synthesis agent produces coherent output. However, the final reports cover only visual arts, completely missing music, writing, and film production. The coordinator\'s logs show it decomposed the topic into three subtasks: "AI in digital art creation," "AI in graphic design," and "AI in photography." What is the most likely root cause?',
    options: [
...
const REFERENCE = [
  { label: 'CCA-F Exam Guide (Anthropic, Feb 2025)', url: 'https://www.anthropic.com/' }
];
...
async function main() {
  ...
  if (existing > 0) {
    console.log(`Exam already has ${existing} questions — skipping question seed.`);
    console.log(`Delete them first to re-seed: DELETE FROM "Question" WHERE "examId" = '${exam.id}';`);
    return;
  }
  ...
  console.log(`✓ CCA-F exam ready (${EXAM_SLUG})`);
  console.log(`  ${QUESTIONS.length} questions seeded, ${Math.min(5, QUESTIONS.length)} flagged as teasers`);
}
