/* Generate the HashiCorp Terraform Associate (003) question-only top-up.
 *
 * Codes match: local == prod == TFA003-P{n}. Same safe shape as the DCA
...
const T = '$tf$';
const CODES = ['TFA003-P1', 'TFA003-P2', 'TFA003-P3'];
...
async function main() {
  const out = [
    '-- HashiCorp Terraform Associate (003) question-only top-up (Wave 2 -> prod).',
    '-- Codes match: local == prod == TFA003-P{n}.',
  ...
  const ts = '20260519150000';
  const dir = path.join('prisma', 'migrations', `${ts}_terraform_topup`);
