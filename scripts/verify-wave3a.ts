/* Verify Wave 3a: 12 new certs, 65 Q x 3 variants, published, clean keys,
...
const CERTS: Record<string, string> = {
  'microsoft-az-305': 'az305',
  'microsoft-az-204': 'az204',
  'microsoft-sc-900': 'sc900',
  'comptia-a-plus': 'aplus',
  'comptia-network-plus': 'networkplus',
  'comptia-security-plus': 'securityplus',
  'google-professional-cloud-architect': 'gcp-pca',
  'google-professional-data-engineer': 'gcp-pde',
  'isc2-ccsp': 'ccsp',
  'linuxfoundation-cks': 'cks',
  'pmi-capm': 'capm',
  'scrum-org-pspo-i': 'pspo'
};

async function main() {
  const norm = (s: string) => (s || '').toLowerCase().replace(/\s+/g, ' ').trim();
  let problems = 0;
  for (const base of Object.keys(CERTS)) {
