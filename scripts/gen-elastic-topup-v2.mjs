/* Generate the Elastic Certified Engineer question-only top-up — V2.
 *
 * CORRECTIVE migration. The original 20260519130000_elastic_topup was
 * committed targeting the wrong code (CERTIFIED-ENGINEER-P*) then edited
 * in place — Prisma migrations are immutable once applied, so the fix
 * never re-ran. This is a NEW migration folder so Prisma runs it fresh.
 * Idempotent (NOT EXISTS), so safe even if the old one inserted anything.
 *
 * Codes now aligned: local == prod == ECE-P{n}.
 *
...
async function main() {
  const out = [
    '-- Elastic Certified Engineer question-only top-up — V2 (corrective).',
    '-- Supersedes 20260519130000_elastic_topup (was edited in place after',
    '-- being committed; Prisma never re-ran it). Codes: local==prod==ECE-P{n}.',
  ...
  const ts = '20260519160000';
  const dir = path.join('prisma', 'migrations', `${ts}_elastic_topup_v2`);
