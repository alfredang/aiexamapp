/* Red Hat RHCSA EX200 question-only top-up migration.
 *
 * Codes match: local == prod == RHCSA-P{n}. Same safe shape as DCA top-up:
 * INSERT INTO "Question" only, idempotent via NOT EXISTS(examId,stem),
 * no Exam/Bundle/Vendor writes, never touches published/deletedAt, no
 * deletes. status = PUBLISHED. (prod RHCSA is ACTIVE; additive only —
 * never removes — so it just enlarges the pool to the curated 65.)
 */
...
const T = '$rh$';
const CODES = ['RHCSA-P1', 'RHCSA-P2', 'RHCSA-P3'];
...
async function main() {
  const out = [
    '-- Red Hat RHCSA EX200 question-only top-up (Wave 2 content -> prod).',
    '-- Codes match: local == prod == RHCSA-P{n}.',
    ...
    '-- prod RHCSA is ACTIVE — additive only (never removes), so this simply',
    '-- enlarges the pool to the curated 65 per variant.',
    '',
  ...
  const ts = '20260519180000';
  const dir = path.join('prisma', 'migrations', `${ts}_rhcsa_topup`);
